...
const STOCK_INFO = {
  ...
  'nvda':  { display: 'NVDA',  name: 'NVIDIA Corp.',        tv: 'NASDAQ:NVDA',  icon: '▲',  color: '#76B900', decimals: 2, related: ['aapl', 'msft', 'meta'] },
  'meta':  { display: 'META',  name: 'Meta Platforms Inc.', tv: 'NASDAQ:META',  icon: '∞',  color: '#0668E1', decimals: 2, related: ['googl', 'msft', 'aapl'] },
  'tsla':  { display: 'TSLA',  name: 'Tesla Inc.',          tv: 'NASDAQ:TSLA',  icon: 'T',  color: '#E31937', decimals: 2, related: ['amzn', 'nvda', 'aapl'] },
};
...
export default function StockSymbolPage({ params }) {
  const symbol = params.symbol.toLowerCase();
  const info = STOCK_INFO[symbol] || { display: symbol.toUpperCase(), name: symbol.toUpperCase(), tv: `NASDAQ:${symbol.toUpperCase()}`, icon: '📈', color: '#60c8d4', decimals: 2, related: [] };
  const botSymbol = symbol.toUpperCase();
      ...
      <div style={{ background: 'linear-gradient(180deg, #0d1f2d 0%, #080d14 100%)', borderBottom: '1px solid #1a2535', padding: '20px 24px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <div style={{ fontSize: '12px', color: '#475569', marginBottom: '8px' }}>
            ...
            {' › '}
            <Link href="/stocks" style={{ color: '#60c8d4', textDecoration: 'none' }}>Stocks</Link>
            {' › '}
            <span>{info.display}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: info.color + '20', border: `2px solid ${info.color}60`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', color: info.color, fontWeight: '700' }}>
                {info.icon}
              </div>
              <div>
          ...
          {signalChecked && signal && (
            <div style={{ background: '#0d1520', border: `1px solid ${rc?.border || '#1a2535'}`, borderRadius: '10px', padding: '20px', marginBottom: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <span style={{ background: rc?.bg, color: rc?.color, border: `1px solid ${rc?.border}`, borderRadius: '4px', padding: '4px 12px', fontSize: '12px', fontWeight: '700' }}>{signal.score}/10</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '14px' }}>
                {[
          ...
          <div style={{ background: '#0d1520', border: '1px solid #1a2535', borderRadius: '10px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '12px' }}>
              {articles.length > 0 ? articles.map((article, i) => (
                <Link key={i} href={`/articles/${article.slug}`} style={{ textDecoration: 'none' }}>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ color: info.color, fontWeight: '700', fontSize: '13px' }}>{info.display}</span>
                ...
                <div style={{ color: '#475569', fontSize: '13px' }}>No signal articles yet for {info.display}. Articles are auto-published when the bot fires a signal.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ background: '#0d1520', border: '1px solid #1a2535', borderRadius: '10px', marginBottom: '20px', overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid #1a2535' }}>
              <div style={{ fontSize: '13px', fontWeight: '600', color: '#f1f5f9' }}>Related Stocks</div>
            </div>
            <div style={{ padding: '8px 0' }}>
              {info.related.map(rel => {
                const relInfo = STOCK_INFO[rel];
                if (!relInfo) return null;
                return (
                  <Link key={rel} href={`/stocks/${rel}`} style={{ textDecoration: 'none' }}>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{ color: relInfo.color, fontSize: '16px', fontWeight: '700' }}>{relInfo.icon}</span>
                        <div>
