...
const CRYPTO_INFO = {
  ...
  'ethusd': { display: 'ETH/USD', name: 'Ethereum / US Dollar', icon: 'Ξ', color: '#627EEA', decimals: 2, tv: 'BITSTAMP:ETHUSD', related: ['btcusd', 'solusd', 'adausd'] },
  'xrpusd': { display: 'XRP/USD', name: 'Ripple / US Dollar', icon: '✕', color: '#00AAE4', decimals: 4, tv: 'BITSTAMP:XRPUSD', related: ['btcusd', 'adausd', 'bnbusd'] },
  'solusd': { display: 'SOL/USD', name: 'Solana / US Dollar', icon: '◎', color: '#9945FF', decimals: 2, tv: 'COINBASE:SOLUSD', related: ['btcusd', 'ethusd', 'adausd'] },
  'bnbusd': { display: 'BNB/USD', name: 'Binance Coin / USD', icon: '⬡', color: '#F3BA2F', decimals: 2, tv: 'BINANCE:BNBUSDT', related: ['btcusd', 'ethusd', 'xrpusd'] },
      ...
      <div style={{ background: 'linear-gradient(180deg, #0d1f2d 0%, #080d14 100%)', borderBottom: '1px solid #1a2535', padding: '20px 24px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <div style={{ fontSize: '12px', color: '#475569', marginBottom: '8px' }}>
            ...
            {' › '}
            <Link href="/crypto" style={{ color: '#60c8d4', textDecoration: 'none' }}>Crypto</Link>
            {' › '}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#64748b' }}>{info.name} · Crypto</div>
              </div>
              {signal && rc && (
        ...
        <div>
          ...
          <div style={{ background: '#0d1520', border: '1px solid #1a2535', borderRadius: '10px', marginBottom: '20px', overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid #1a2535' }}>
              <div style={{ fontSize: '13px', fontWeight: '600', color: '#f1f5f9' }}>Related Crypto</div>
            </div>
            <div style={{ padding: '8px 0' }}>
              {info.related.map(rel => {
                const relInfo = CRYPTO_INFO[rel];
                if (!relInfo) return null;
                return (
                  <Link key={rel} href={`/crypto/${rel}`} style={{ textDecoration: 'none' }}>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '11px', color: '#475569' }}>{relInfo.name.split(' / ')[0]}</div>
                        </div>
                      </div>
