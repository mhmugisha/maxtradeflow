...
const CRYPTO_SYMBOLS = ['BTCUSD', 'ETHUSD', 'XRPUSD', 'SOLUSD', 'BNBUSD', 'ADAUSD'];

const CRYPTO_INFO = {
  'BTCUSD': { display: 'BTC/USD', name: 'Bitcoin / US Dollar', icon: '₿', color: '#F7931A', decimals: 2 },
  'ETHUSD': { display: 'ETH/USD', name: 'Ethereum / US Dollar', icon: 'Ξ', color: '#627EEA', decimals: 2 },
  'XRPUSD': { display: 'XRP/USD', name: 'Ripple / US Dollar', icon: '✕', color: '#00AAE4', decimals: 4 },
  'SOLUSD': { display: 'SOL/USD', name: 'Solana / US Dollar', icon: '◎', color: '#9945FF', decimals: 2 },
  'BNBUSD': { display: 'BNB/USD', name: 'Binance Coin / USD', icon: '⬡', color: '#F3BA2F', decimals: 2 },
  'ADAUSD': { display: 'ADA/USD', name: 'Cardano / US Dollar', icon: '₳', color: '#0033AD', decimals: 3 },
};

const FILTERS = ['All Crypto', 'Large Cap', 'Mid Cap'];
const LARGE_CAP = ['BTCUSD', 'ETHUSD', 'BNBUSD'];
const MID_CAP = ['XRPUSD', 'SOLUSD', 'ADAUSD'];
const PAGE_SIZE = 6;
...
export default function CryptoPage() {
  ...
  const [filter, setFilter] = useState('All Crypto');
  ...
  useEffect(() => {
    ...
    fetch('/api/articles?category=signal&limit=6')
      .then(r => r.json())
      .then(d => setRecentSignals((d.articles || []).filter(a => CRYPTO_SYMBOLS.includes(a.ticker))))
      .catch(() => {});
  ...
  const cryptoPrices = prices.filter(p => CRYPTO_SYMBOLS.includes(p.symbol));
  const cryptoScreener = screener.filter(s => CRYPTO_SYMBOLS.includes(s.symbol));
  const tradeSignals = cryptoScreener.filter(s => s.action === 'TRADE').length;

  const filteredPrices = cryptoPrices.filter(p => {
    if (filter === 'All Crypto') return true;
    if (filter === 'Large Cap') return LARGE_CAP.includes(p.symbol);
    if (filter === 'Mid Cap') return MID_CAP.includes(p.symbol);
    return true;
  });
  ...
  const getSignal = (symbol) => cryptoScreener.find(s => s.symbol === symbol);
  const getInfo = (symbol) => CRYPTO_INFO[symbol] || { display: symbol, name: symbol, icon: '🪙', color: '#60c8d4', decimals: 2 };
      ...
      <div style={{ background: 'linear-gradient(180deg, #0d1f2d 0%, #080d14 100%)', borderBottom: '1px solid #1a2535', padding: '28px 24px 20px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              <h1 style={{ fontSize: '28px', fontWeight: '700', color: '#f1f5f9', margin: '0 0 4px' }}>Crypto Markets</h1>
              <p style={{ color: '#64748b', fontSize: '13px', margin: 0 }}>BTC · ETH · XRP · SOL · BNB · ADA · Live prices & AI signal scores</p>
            </div>
            <div style={{ display: 'flex', gap: '12px' }}>
              {[
                { label: 'Pairs Tracked', value: '6', color: '#60c8d4' },
                { label: 'Active Signals', value: tradeSignals.toString(), color: '#1D9E75' },
                { label: 'Strongest', value: 'BTC', color: '#F7931A' },
              ].map(card => (
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '24px' }}>
            {['BTCUSD', 'ETHUSD', 'SOLUSD'].map(symbol => {
              const price = cryptoPrices.find(p => p.symbol === symbol);
              const signal = getSignal(symbol);
              const info = getInfo(symbol);
              const rc = signal ? ratingColor(signal.action) : null;
              return (
                <Link key={symbol} href={`/crypto/${symbol.toLowerCase()}`} style={{ textDecoration: 'none' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '10px', color: '#475569' }}>{info.name.split(' / ')[0]}</div>
                        </div>
                      </div>
                      {signal && rc && (
          ...
          <div style={{ background: '#0d1520', border: '1px solid #1a2535', borderRadius: '10px', marginBottom: '24px', overflow: 'hidden' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #1a2535' }}>
                    {['Asset', 'Bid', 'Ask', 'Spread', 'ADX', 'Score', 'Direction', 'Signal', ''].map(h => (
                ...
                <tbody>
                  {pagedPrices.length > 0 ? pagedPrices.map((price, i) => {
                    ...
                    const rc = signal ? ratingColor(signal.action) : null;
                    const info = getInfo(price.symbol);
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: '12px', color: '#e05555', fontFamily: 'monospace', fontWeight: '500' }}>${parseFloat(price.bid).toFixed(info.decimals)}</td>
                        <td style={{ padding: '12px', color: '#1D9E75', fontFamily: 'monospace' }}>${parseFloat(price.ask).toFixed(info.decimals)}</td>
                        ...
                        <td style={{ padding: '12px' }}>
                          <Link href={`/crypto/${price.symbol.toLowerCase()}`} style={{ textDecoration: 'none' }}>
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderTop: '1px solid #1a2535' }}>
                <div style={{ fontSize: '12px', color: '#475569' }}>Showing {((page - 1) * PAGE_SIZE) + 1}–{Math.min(page * PAGE_SIZE, filteredPrices.length)} of {filteredPrices.length}</div>
          ...
          <div style={{ background: '#0d1520', border: '1px solid #1a2535', borderRadius: '10px', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid #1a2535' }}>
              <div style={{ fontSize: '13px', fontWeight: '600', color: '#f1f5f9' }}>Recent Crypto Signals</div>
            ...
            <div style={{ padding: '16px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '12px' }}>
              {recentSignals.length > 0 ? recentSignals.map((article, i) => (
                ...
                <div style={{ color: '#475569', fontSize: '13px' }}>Crypto signal articles will appear here as the bot fires signals.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ background: '#0d1520', border: '1px solid #1a2535', borderRadius: '10px', marginBottom: '20px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '8px 0' }}>
              {cryptoScreener.filter(s => s.action === 'TRADE').slice(0, 5).map((signal, i) => {
              ...
              {cryptoScreener.filter(s => s.action === 'TRADE').length === 0 && (
