...
const SIGNAL_FIELDS = `
  ...
  market_condition, session, expected_duration, reasons,
 ...
 */
export async function fetchSignalsPage(sql, { page = 1, perPage = 20, assetClass = null, status = null, classTickers = null } = {}) {
...
/** The article published for a signal (slug + excerpt), or null. */
export async function fetchArticleForSignal(sql, uid) {
  const rows = await sql.query(
    `SELECT slug, excerpt FROM articles WHERE signal_uid = $1 AND published = true LIMIT 1`,
    [uid]
  );
  return rows[0] ?? null;
}
